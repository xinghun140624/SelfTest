...
@interface HCForgetPassWordView ()<UITextFieldDelegate>
...
@implementation HCForgetPassWordView

- (void)textFieldDidChange:(UITextField *)textField {
    if (_mobileTextField.text.length>11) {
        _mobileTextField.text = [_mobileTextField.text substringToIndex:11];
    }else if (_mobileTextField.text.length==11 && self.imageCodeTextField.text.length >0 && self.messageCodeTextField.text.length >0){
...
- (void)nextButtonClick:(UIButton *)button {
    ...
    if (![HCLoginTool isValidPhoneNum:self.mobileTextField.text]) {
        [MBProgressHUD showText:@"手机号输入有误，请重新输入！"];
    ...
    if ([self.delegate respondsToSelector:@selector(forgetPasswordView:nextButtonClick:)]) {
        [self.delegate forgetPasswordView:self nextButtonClick:button];
    }
}
-(void)codeButtonClick:(UIButton *)button {
    ...
    HCMobileCaptchaApi * api = [[HCMobileCaptchaApi alloc] initWithBusiness:1 mobile:self.mobileTextField.text picCaptcha:self.imageCodeTextField.text type:0];

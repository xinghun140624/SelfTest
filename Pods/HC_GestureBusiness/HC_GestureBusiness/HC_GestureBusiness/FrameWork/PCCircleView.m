...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    // 数组中最后一个对象的处理
    [self circleSetLastObjectWithState:CircleStateLastOneSelected];
    if([self.delegate respondsToSelector:@selector(circleView:type:connectCirclesingWithGesture:)]){
        [self.delegate circleView:self type:self.type connectCirclesingWithGesture:nil];
    }
    [self setNeedsDisplay];
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    switch (self.type) {
        ...
        case CircleViewTypeVerify:
            ...
            break;
        case CircleViewTypeVerifyAndDelete:
            [self gestureEndByTypeVerifyWithGesture:gesture length:length];
            break;
        default:
            [self gestureEndByTypeSettingWithGesture:gesture length:length];
...
- (void)gestureEndByTypeLoginWithGesture:(NSString *)gesture length:(CGFloat)length
{
    ...
    [HCSettingGestureService getUserGesture:^(BOOL success, NSString *password) {
       
        if (!success) {
            [MBProgressHUD showText:@"网络连接错误，请稍后重试"];
            return;
        }
        if (success && gesture) {
